...
/*   Updated: 2025/02/26 15:02:55 by carlopez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_build_lines(char *char_map, matrix_map *map)
{
	...
	j = 0;
	fix = 0;
	ft_printf("Entra en build lines \n");
	while (j < map->height)
	{
		(map->point)[j] = (matrix_point *)malloc(map->width * sizeof(matrix_point));
		if (!(map->point)[j])
			return ;
